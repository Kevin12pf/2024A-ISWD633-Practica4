...
Se han vuelto a evaluar los 5 pasos, pero lo diferente es que Docker utilizó el caché (verás que dice CACHED junto al comando). Solamente a partir del momento que copiamos el archivo (COPY), como observó que index.html fue modificado, invalidó el escaneo de caché y procedió a compilar nuevamente desde ahí hasta el final.
...
```
docker run -d -P --name mi-contenedor-apache mi-apache:1.0
```
...
Una imagen huérfana es una imagen de Docker que no está asociada a ningún contenedor en ejecución. Es decir, es una imagen que ha sido creada pero no se ha utilizado para crear un contenedor, o bien, el contenedor que la utilizaba ha sido eliminado.
